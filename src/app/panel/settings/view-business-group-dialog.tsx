...
interface ViewBusinessGroupDialogProps {
  ...
  onUpdate: (businessGroup: BusinessGroup) => void;
}

export function ViewBusinessGroupDialog({
  businessGroup,
  open,
  onOpenChange,
  onUpdate,
  ...
  const updateMutation = api.businessGroup.update.useMutation({
    ...
    onError: (error) => {
      console.error(error);
      toast.error('Meslek grubu güncellenirken bir hata oluştu');
    },
  });

  const deleteMutation = api.businessGroup.delete.useMutation({
    onSuccess: () => {
      utils.businessGroup.get.invalidate();
      toast.success('Meslek grubu başarıyla silindi');
      handleOpenChange(false);
    },
    onError: (error) => {
      console.error(error);
      toast.error('Meslek grubu silinirken bir hata oluştu');
    },
  });
  ...
  const handleDelete = async () => {
    await deleteMutation.mutateAsync({ id: businessGroup.id });
  };
  ...
  return (
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent
        aria-describedby="Meslek grubu görüntüleme ve düzenleme"
      ...
      >
        <DialogHeader>
          ...
          {!isEditMode && (
            <DialogDescription>
              Meslek grubu bilgilerini görüntüleyin veya düzenleyin
            </DialogDescription>
          )}
        </DialogHeader>
        ...
        {showDeleteConfirm ? (
          <div className="space-y-4 py-4">
            <p className="text-center font-medium text-lg">
              Bu meslek grubunu silmek istediğinizden emin misiniz?
            </p>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm">
                  {new Date(businessGroup.createdAt).toLocaleString('tr-TR')}
                </p>
              </div>
              <div>
                <Label className="text-muted-foreground">
                ...
                <p className="text-sm">
                  {new Date(businessGroup.updatedAt).toLocaleString('tr-TR')}
                </p>
              </div>
            </div>
            <DialogFooter>
